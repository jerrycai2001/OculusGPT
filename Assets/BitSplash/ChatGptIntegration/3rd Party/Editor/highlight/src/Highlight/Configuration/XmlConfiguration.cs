...
namespace Highlight.Configuration
{
    public class XmlConfiguration : IConfiguration
    {
        ...
        static bool TryParseHtmlString(string str, out Color foreColor)
        {
            ...
            if (ColorUtility.TryParseHtmlString(str, out foreColor))
                return true;
            string val;
            if (colorCodes.TryGetValue(str, out val) == false)
                return false;
            return ColorUtility.TryParseHtmlString(val, out foreColor);
        }
        private ColorPair GetPatternColors(XElement fontElement)
        {
            Color foreColor;
            if (!TryParseHtmlString(fontElement.GetAttributeValue("foreColor"), out foreColor))
                throw new Exception();
            return new ColorPair(foreColor);
        }
